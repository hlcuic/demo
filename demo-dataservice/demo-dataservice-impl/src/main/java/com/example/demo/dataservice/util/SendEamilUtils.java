package com.example.demo.dataservice.util;
...
public class SendEamilUtils {
	...
	static String FROM = "chl2009.hi@163.com"; // 发件人地址
	static String TO = "1271784290@qq.com"; // 收件人地址
	static String AFFIX = "classpath:config/template/abc.doc"; // 附件地址
	static String USER = "chl2009.hi@163.com"; // 用户名
	static String PWD = "86128527chl"; // 163的授权码
	static String SUBJECT = "hello,emaildemo"; // 邮件标题
	...
	public static void send(String context) {
		...
		try {
			...
			if (!AFFIX.isEmpty()) {// 添加附件
				...
				AFFIX = AFFIX.substring("classpath:".length());
				URL url = Thread.currentThread().getContextClassLoader().getResource(AFFIX);
				
				DataSource source = new FileDataSource(url.getFile());
				DataHandler handler = new DataHandler(source);
				messageBodyPart.setDataHandler(handler);// 添加附件的内容
				messageBodyPart.setFileName(handler.getName());
